...
const CustomDatePicker = ({ selectedDate, onChange }) => {
  ...
  const days = [];
  for (let i = 0; i < startOffset; i++) days.push(<div key={`empty-${i}`} className="w-8 h-8 md:w-10 md:h-10"></div>);
  
  for (let i = 1; i <= daysInMonth; i++) {
    const tileDate = new Date(currentMonth.getFullYear(), currentMonth.getMonth(), i);
    const isPast = tileDate < today;
    const isTuesday = tileDate.getDay() === 2;
    const isDisabled = isPast || isTuesday;
    const isSelected = tileDate.toDateString() === currentSelectedDate.toDateString();

    days.push(
      <button key={i} disabled={isDisabled} onClick={(e) => { e.stopPropagation(); handleSelect(i); }}
        className={cn("w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full text-sm font-bold transition-all",
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }}
            ...
            <div className="grid grid-cols-7 gap-1 mb-4">
              {['L','M','X','J','V','S','D'].map((d, idx) => (
                <div key={d} className={cn("w-8 h-8 md:w-10 md:h-10 flex items-center justify-center text-xs font-black uppercase", idx === 1 ? "text-neutral-dark/20 line-through" : "text-sierra-gold/50")}>{d}</div>
              ))}
            </div>
...
export default function BookingAgent({ isOpen, onClose }) {
  ...
  const handleWhatsApp = () => {
    const message = `*Solicitud de Reserva — Las Olivillas*%0A%0A%F0%9F%93%85 *Fecha:* ${booking.date}%0A%E2%8F%B0 *Hora:* ${booking.time}%0A%F0%9F%91%A5 *Personas:* ${booking.adults}%0A%F0%9F%93%8D *Zona:* ${booking.area.toUpperCase()}%0A%F0%9F%91%A4 *Nombre:* ${booking.name}%0A%F0%9F%93%9E *Teléfono:* ${booking.phone}`;
    window.open(`https://wa.me/34653999909?text=${message}`, '_blank');
  };

  const handleWhatsAppDirect = () => {
    window.open(`https://wa.me/34653999909?text=Hola,%20me%20gustar%C3%ADa%20hacer%20una%20reserva%20en%20Las%20Olivillas.`, '_blank');
  };
          ...
          <motion.div
            ...
            <div className="flex justify-between items-center px-6 py-4 md:px-10 md:py-6 border-b border-black/5 shrink-0 rounded-t-[2rem] md:rounded-t-[3rem]">
              <div className="flex items-center gap-6 md:gap-12">
                <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.4em] md:tracking-[0.6em] text-neutral-dark/40">Reservar Mesa</span>
              ...
              <AnimatePresence mode="wait">
                {phase === 1 && (
                  <motion.div key="p1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col items-center text-center">
                    <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif font-black mb-8 md:mb-12">¿Cuándo nos <span className="italic font-normal text-terracotta-mid">visitas?</span></h2>
                    <div className="grid md:grid-cols-3 gap-6 md:gap-8 w-full max-w-4xl mb-8 relative">
                      <div className="flex flex-col items-start gap-3 relative z-[200]">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-dark/40 ml-2">Fecha</span>
                ...
                {phase === 2 && (
                  <motion.div key="p2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col items-center">
                    <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif font-black mb-8 md:mb-12">Elige tu <span className="italic font-normal text-terracotta-mid">zona.</span></h2>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-8 w-full max-w-3xl">
                      {['Interior', 'Terraza', 'Barra'].map(area => (
                        <div key={area} onClick={() => setBooking({...booking, area: area.toLowerCase()})}
                ...
                {phase === 3 && (
                  <motion.div key="p3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col items-center max-w-md mx-auto w-full text-center">
                    ...
                    <p className="text-neutral-dark/60 font-serif italic mb-8 md:mb-12 text-sm md:text-base">Déjanos tu nombre y teléfono para confirmar la reserva.</p>
            ...
            <div className="px-6 py-6 md:px-10 md:py-8 flex justify-between items-center bg-white border-t border-black/5 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.03)] rounded-b-[2rem] md:rounded-b-[3rem] relative z-0">
              ...
              {phase < 3 ? (
                ...
                <button onClick={handleWhatsApp} className="group flex items-center gap-4 md:gap-6 text-[#25D366]">
                  <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] md:tracking-[0.5em] mt-1">Confirmar por WhatsApp</span>
